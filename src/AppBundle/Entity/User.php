 ...
 * @UniqueEntity(fields="email", message="抱歉，该Email已经被使用",groups={"Account"})
 */
class User implements UserInterface, \Serializable
{
     ...
     *      min = 6,
     *      max = 20,
     *      minMessage = "不能少于 {{ limit }} 字符长度",
     *      maxMessage = "不能大于 {{ limit }} 字符长度",
     *      groups = {"Account"}
     * )
     * @Assert\NotBlank(message = "密码不能为空.", groups={"Account"})
     * @ORM\Column(type="string", length=64)
     */
    private $password;

    /**
     * @Assert\NotBlank(message = "Email不能为空.", groups={"Account"})
     * @Assert\Email(message = "Email不符合规则.", groups={"Account"})
